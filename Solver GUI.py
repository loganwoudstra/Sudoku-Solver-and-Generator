...
class Sudoku:
    def __init__(self, master):
        ...
        for y in range(1, 10):
            ...
            for x in range(1, 10):
                ...
                square['font'] = ('Arial', 20, 'bold italic')
                new_row.append(square)
            self.squares.append(new_row)

        # creates buttons
        self.button("Solve Board", 0, 0.86, self.correct_board)
        self.button("Clear Board", 0, 0.93, self.clear)
        self.button("New Puzzle", 0.66, 0.86, self.new_puzzle)
        self.button("Check Puzzle", 0.66, 0.93, self.check_sudoku)

        # places timer
        self.timer = tk.Label(self.frame, text="0:00", bg='light gray', width=5, font='Arial 28')
        self.timer.place(relx=0.5, rely=0.995, anchor='s')

    def button(self, text, relx, rely, command):
        button = tk.Button(self.frame, text=text, highlightthickness=0, highlightbackground='light gray',
                           width=10, command=lambda: command())
    ...
    def new_puzzle(self):  # creates a new solvable puzzle
        ...
        for y, x in itertools.product(range(9), repeat=2):
            if self.real_numbers[y][x] == 0:
                ...
                self.squares[y][x].config(font='Arial 20 bold italic', disabledforeground='black', state='disabled')
    ...
    def clear(self):  # clears the entire board
        for y, x in itertools.product(range(9), repeat=2):
            self.squares[y][x].config(font='Arial 20 bold italic', fg='black', state='normal')
            self.display_numbers[y][x].set('')
            self.real_numbers[y][x] = 0
    ...
    def solve(self, multiple_solutions=False, forbidden_value=None):  # finds solution to current board
        ...
        for n in range(1, 10):
            ...
            if self.isvalid(y, x, n):
                self.squares[y][x].config(font='Arial 20', fg='dark blue')
